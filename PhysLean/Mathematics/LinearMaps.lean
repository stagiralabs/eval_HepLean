...
/-- The structure defining a homogeneous quadratic equation. -/
@[simp]
def HomogeneousQuadratic (V : Type) [AddCommMonoid V] [Module ℚ V] : Type := by sorry
...
@[target] lemma map_smul (f : HomogeneousQuadratic V) (a : ℚ) (S : V) : f (a • S) = a ^ 2 * f S := by sorry
...
@[target] lemma swap (f : BiLinearSymm V) (S T : V) : f S T = f T S := by sorry
...
@[target] lemma toHomogeneousQuad_add {V : Type} [AddCommMonoid V] [Module ℚ V]
...
/-- The structure of a homogeneous cubic equation. -/
@[simp]
def HomogeneousCubic (V : Type) [AddCommMonoid V] [Module ℚ V] : Type := by sorry
...
@[target] lemma toCubic_add {charges : Type} [AddCommMonoid charges] [Module ℚ charges]

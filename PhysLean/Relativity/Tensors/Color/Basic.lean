...
@[target] lemma mk_hom (f : X → C) : (mk f).hom = f := by sorry
...
/-- The equivalence of types underlying the identity morphism is the reflexive equivalence. -/
@[target] lemma toEquiv_id (f : OverColor C) : toEquiv (𝟙 f) = Equiv.refl f.left := by
  sorry
...
@[target] lemma toEquiv_symm_apply (m : f ⟶ g) (i : g.left) :
    f.hom ((toEquiv m).symm i) = g.hom i := by
  sorry


@[target] lemma toEquiv_comp_hom (m : f ⟶ g) : g.hom ∘ (toEquiv m) = f.hom := by
  sorry


@[target] lemma toEquiv_comp_inv_apply (m : f ⟶ g) (i : g.left) :
...
@[target] lemma hom_comp (m : f ⟶ g) (n : g ⟶ h) : (m ≫ n).hom = m.hom ≫ n.hom := by sorry

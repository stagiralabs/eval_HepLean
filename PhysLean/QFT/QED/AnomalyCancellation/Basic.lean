...
/-- The vector space of charges. -/
@[simps!]
def PureU1Charges (n : ℕ) : ACCSystemCharges := by sorry
...
/-- A solution to the pure U(1) accs satisfies the linear ACCs. -/
@[target] lemma pureU1_linear {n : ℕ} (S : (PureU1 n).LinSols) :
    ∑ i, S.val i = 0 := by
  sorry
...
/-- The last charge of a solution to the linear ACCs is equal to the negation of the sum
  of the other charges. -/
@[target] lemma pureU1_last {n : ℕ} (S : (PureU1 n.succ).LinSols) :
...
/-- The `j`th charge of a sum of pure-`U(1)` charges is equal to the sum of
  their `j`th charges. -/
@[target] lemma sum_of_charges {n : ℕ} (f : Fin k → (PureU1 n).Charges) (j : Fin n) :
...
/-- The `j`th charge of a sum of solutions to the linear ACC is equal to the sum of
  their `j`th charges. -/
@[target] lemma sum_of_anomaly_free_linear {n : ℕ} (f : Fin k → (PureU1 n).LinSols) (j : Fin n) :
